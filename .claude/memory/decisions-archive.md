...
tags: [memory, architecture, archive]
updated: [2026-08-12]
---

# Decisioni Architetturali — Archivio (2026-08-04 → 2026-08-10)
> Voci spostate qui da `decisions.md` il 2026-08-12 perché il file attivo aveva superato i 160KB
> (oltre la soglia di 100KB per la lettura diretta). **Questo file non è auto-caricato** in
> sessione (nessun `@` in MEMORY.md, solo il wikilink [[decisions-archive]]) — leggerlo on-demand
> con Grep per individuare la sezione, poi Read con offset+limit, o direttamente con `sed`/`grep`
> da shell. Decisioni correnti/recenti (dal 2026-08-11 in poi) restano in [[decisions]].
> Ordine cronologico invariato, nessun contenuto riscritto o accorciato in questo spostamento.
