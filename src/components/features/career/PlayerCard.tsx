...
export function PlayerCard({
  ...
  // Su mobile il cartellino apre di default solo gli essenziali (nome/club/età/OVR/obiettivo);
  // il resto (valore, attributi, record, trofei) sta dietro questo toggle. A `lg:` (layout
  // desktop a 3 colonne) resta sempre espanso, il toggle stesso è nascosto — vedi wrapper sotto.
    ...
    >
      ...
      {compact ? (
        <button
          ...
          className="flex items-center gap-1 self-start text-xs font-semibold tracking-wide text-(--color-text-muted) uppercase lg:hidden"
        >
          <ChevronDown
            size={14}
            aria-hidden="true"
      ...
      >
        ...
        >
          <div className="rounded-lg bg-(--color-surface) py-2">
            <p
              className={cn(
                ...
                (
                  isGoalkeeper
                    ? recordsFlashKey.includes("clean sheet")
                    : recordsFlashKey.includes("gol")
                )
                  ? "text-(--color-ovr-gold)"
                  : "text-(--color-text)"
              )}
            >
              {isGoalkeeper
                ? player.records.bestSeasonCleanSheets ?? 0
          ...
          <div className="rounded-lg bg-(--color-surface) py-2">
            <p
              className={cn(
                "font-display text-lg",
                recordsFlashKey.includes("assist")
                  ? "text-(--color-ovr-gold)"
                  : "text-(--color-text)"
              )}
            >
          ...
          <div className="rounded-lg bg-(--color-surface) py-2">
            <p
              className={cn(
                "font-display text-lg",
                recordsFlashKey.includes("presenze")
                  ? "text-(--color-ovr-gold)"
                  : "text-(--color-text)"
              )}
            >
        ...
        >
          <span
            ...
            title={
              trophyCount > 0
                ? `${trophyCount} trofei vinti`
                : "Nessun trofeo vinto finora"
            }
          >
            <TrophyIcon
              size={14}
          ...
          <span
            ...
            title={
              awardCount > 0
                ? `${awardCount} premi individuali`
                : "Nessun premio individuale finora"
            }
          >
            <AwardIcon
              size={14}
