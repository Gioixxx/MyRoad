...
import { computeCoachHallOfFame } from "@/lib/coach-career/coach-satisfaction";
import { Button } from "@/components/ui/Button";
import { CountryFlag } from "@/components/features/career/CountryFlag";
...
export function CoachArchive({ entries, onBack }: CoachArchiveProps) {
  const hof = computeCoachHallOfFame(entries);
  ...
  return (
    <div className="flex min-w-0 flex-col items-center gap-5 text-center">
      <div>
        ...
        <h2 className="font-display text-2xl text-(--color-text)">Le mie carriere da allenatore</h2>
      </div>
      ...
      {entries.length > 0 ? (
        <div className="w-full min-w-0 chalk-panel rounded-xl p-4 text-left">
          ...
          <ul className="grid gap-2 sm:grid-cols-2">
            <HofRow
              label="Reputazione più alta"
              entry={hof.highestReputation}
              detail={hof.highestReputation ? `Rep. ${hof.highestReputation.peakReputation}` : null}
            />
            <HofRow
              label="Più trofei"
      ...
      {entries.length === 0 ? (
        <div className="flex flex-col items-center gap-1.5 rounded-lg border border-dashed border-(--color-border) px-6 py-8 text-center">
          ...
          <p className="text-sm text-(--color-text-muted)">Nessuna carriera allenatore conclusa finora.</p>
        </div>
      ) : (
        <div className="w-full min-w-0 chalk-panel rounded-xl text-left">
          <ul className="divide-y divide-(--color-border)">
            {entries.map((entry) => {
              ...
              const isHof =
                entry.id === hof.highestReputation?.id ||
              ...
              return (
                ...
                >
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <OvrBadge ovr={entry.peakReputation} size="sm" />
                    {isHof ? (
                  ...
                  <p className="pl-6 text-xs text-(--color-text-muted) sm:pl-0">
                    ...
                    Ritirato a {entry.retiredAge} anni · {entry.clubsManaged}{" "}
                    {entry.clubsManaged === 1 ? "club allenato" : "club allenati"} · {entry.trophyCount}{" "}
...
function HofRow({
  ...
  label: string;
  entry: ArchivedCoachCareer | null;
  detail: string | null;
}) {
  return (
